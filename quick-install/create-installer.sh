...
#1. please make sure that the latest neon-server docker image (neon-api.tar.gz) is copied to 
#   the quick-install folder. This file is an archive of the neon-server image pulled from docker. Consult the
#   neon-server documentation on how to dockerize neon-server. After dockerizing neon-server, use the following
#   command to save the image as a zip archive and copy the zip archive to the quick-install folder
