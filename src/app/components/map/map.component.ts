...
export class MapComponent extends BaseLayeredNeonComponent implements OnInit, OnDestroy, AfterViewInit, FilterListener {
    ...
    getExportFields(layerIndex: number): any[] {
        let usedFields = [
            this.options.layers[layerIndex].idField,
            this.options.layers[layerIndex].latitudeField,
            ...
            if (latCoord instanceof Array && lngCoord instanceof Array) {
                for (let pos = latCoord.length - 1; pos >= 0; pos--) {
                    ...
                    if (hoverPopupValue instanceof Array) {
                        this.addOrUpdateUniquePoint(map, idValue, latCoord[pos], lngCoord[pos], colorField, colorValue, hoverPopupValue[pos]);
                    } else {
                        this.addOrUpdateUniquePoint(map, idValue, latCoord[pos], lngCoord[pos], colorField, colorValue, hoverPopupValue);
                    }
                }
            } else {
    ...
    */
    ...
    addOrUpdateUniquePoint(map: Map<string, UniqueLocationPoint>, idValue, lat: number, lng: number, colorField: string, colorValue: string,
