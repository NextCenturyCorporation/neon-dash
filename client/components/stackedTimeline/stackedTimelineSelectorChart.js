 ...
 * @constructor
 */
charts.StackedTimelineSelectorChart = function(element, configuration) {
    ...
    this.render = function(values) {
        ...
        var createSeries = function(series) {
            ...
            if(series.name === me.primarySeries.name) {
                ...
                if(series.type === 'bar' && series.data.length < me.width) {
                    ...
                    .enter().append("rect")
                        ...
                        var getDateFields = function() {
                            var toReturn = [];
                            if(me.granularity === 'day') { toReturn.push('year'); }
                            if(me.granularity === 'month') { toReturn.push('month'); }
                            if(me.granularity === 'day') { toReturn.push('day'); }
                            if(me.granularity === 'hour') { toReturn.push('hour'); }
                            return toReturn;
                        };
                ...
                series.data[0].values.forEach(function(datum, index) {
    ...
    this.updateGroupField = function(groupField) {
    ...
    this.drawFocusChart = function(series) {
        ...
        var dataShown = series.data.map(function(category) {
            var values = _.filter(category.values, function(obj) {
        ...
        me.focusDateToIndex = {};
        if(dataShown.length) {
            dataShown[0].values.forEach(function(datum, index) {
