 ...
 * @constructor
 */
charts.HistogramSelectorChart = function(element, configuration) {
    ...
    this.focusValueToIndex = {};
    this.contextValueToIndex = {};
    ...
    // Bucketizer, pulled in from histogram.controller.js because we can't use d3's built-in time intervals to snap values to buckets.
    this.bucketizer = undefined;
    ...
    this.setBucketizer = function(bucketizer) {
     ...
     * @private
     */
    var wrapBrushHandler = function(histogram, handler) {
        return function() {
            if(histogram.brush) {
                XDATA.userALE.log({
                    activity: "select",
                    action: "dragend",
                    elementId: "histogram-brush",
                    elementType: "canvas",
                    elementSub: "histogram-brush",
                    elementGroup: "chart_group",
                    source: "user",
                    tags: ["histogram", "brush"]
                });

                // If the user clicks on a date inside the brush without moving the brush, change the brush to contain only that date.
                if(histogram.hoverIndex >= 0 && histogram.oldExtent[0]) {
                    var extent = histogram.brush.extent();
                    if(histogram.oldExtent[0] == extent[0] && histogram.oldExtent[1] == extent[1]) {
                        var startValue = histogram.data[0].data[histogram.hoverIndex].bottom;
                        var endValue = histogram.data[0].data.length === histogram.hoverIndex + 1 ? histogram.xDomain[1] : histogram.data[0].data[histogram.hoverIndex + 1].top;
                        histogram.brush.extent([startValue, endValue]);
                    }
                }

                if(handler) {
                    handler(histogram.brush.extent());
                }
            }
        };
    };
    ...
    this.selectIndexedDates = function(startIndex, endIndex) {
        ...
        for(var i = startIndex; i < endIndex; ++i) {
            ...
            var focusIndex = this.focusValueToIndex[primaryData.data[i].date.toUTCString()];
            if(focusIndex >= 0) {
    ...
    this.render = function(values) {
        ...
        // look funny)
        var minimumTickRange = this.bucketizer.getNumBuckets();
        ...
        var createSeries = function(series) {
            ...
            var focusContainer = focus.append("g")
                ...
                .on('mousemove', function() {
                    ...
                    if(index >= 0 && index < series.data.length) {
                        var contextIndex = me.contextValueToIndex[series.data[index].date.toUTCString()];
            ...
            if(series.name === me.primarySeries.name) {
                ...
                me.contextValueToIndex = {};
                series.data.forEach(function(datum, index) {
                    me.contextValueToIndex[datum.date.toUTCString()] = index;
                });
    ...
    this.drawFocusChart = function(series) {
        ...
        me.focusValueToIndex = {};
        dataShown.forEach(function(datum, index) {
            me.focusValueToIndex[datum.date.toUTCString()] = index;
        });
