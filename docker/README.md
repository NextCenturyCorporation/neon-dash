...
| ES_DATA_MAPPING_TUPLE           | _ldc_uyg_jul_18_:_ui_out_, | List of tuple values separated by a semicolon, comma delimiated. Each tuple grouping is the data file name followed by the name of the mapping. The data file name will become the name of the index in elasticsearch as well as locating the .json files for the data and the mapping. This requires the mapping file is always has the prefix of the data file name followed by \_mapping |  
